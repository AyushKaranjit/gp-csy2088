...
try {
  ...
  if (!$auth->isLoggedIn() || !$auth->isAdmin()) { ApiResponse::error('Admin access required', 403); return; }
  $input = json_input();
  $orderId = (int)($input['order_id'] ?? 0);
  $status = trim((string)($input['status'] ?? ''));
  if ($orderId <= 0 || $status === '') { ApiResponse::error('Order ID and status required', 400); return; }
  $valid = ['pending','confirmed','processing','shipped','delivered','cancelled','refunded'];
  if (!in_array($status, $valid, true)) { ApiResponse::error('Invalid status', 400); return; }
  $db = db();

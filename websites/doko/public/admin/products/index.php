...
try {
    ...
    error_log("Admin Products: Loaded " . count($categories) . " categories");

    // Debug: Log first category if available
    if (!empty($categories)) {
        $firstCategory = $categories[0];
        error_log("Admin Products: First category - ID: {$firstCategory['category_id']}, Name: {$firstCategory['name']}");
    }

 ...
 * Renders a slim navigation bar below the global storefront header so admin pages
 * share the same look & footer. Keeps logic minimal; path links are absolute.
 */
$script = $_SERVER['SCRIPT_NAME'] ?? '';
function doko_admin_active(string $section, string $script): string {
    return (strpos($script, "/admin/$section/") !== false) ? 'active' : '';
}
?>
<nav class="admin-subnav">
    <div class="container">
        <div class="admin-brand"><a href="/admin/dashboard/" class="admin-logo-link">DOKO <span>Admin</span></a></div>
        <ul class="admin-subnav-list">
            <li><a class="<?php echo doko_admin_active('dashboard', $script); ?>" href="/admin/dashboard/">Dashboard</a></li>
  ...
  async function fetchCount(){
    try {
      ...
      if(data && typeof data.total === 'number'){
        const el = document.getElementById('admin-products-count');
